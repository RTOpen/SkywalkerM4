...
uint16_t adc_read(uint8_t ch)
{
    ...
    ADC_DMACfg(ENABLE, (uint16_t)(uint32_t)&adc_buffer[0], (uint16_t)(uint32_t)&adc_buffer[ADC_OVERSAMPLE_TIMES-1], ADC_Mode_Single);
    PFIC_EnableIRQ(ADC_IRQn);
    ADC_StartDMA();
    rt_sem_take(&wait_sem, RT_WAITING_FOREVER);
    adc_value = (adc_buffer[0] + adc_buffer[1] + adc_buffer[2] + adc_buffer[3])/4;
    return adc_value;
}
